...
#+BEGIN_SRC emacs-lisp :results silent
  ...
  (defun remove-couple (couple)
    (if (> (length dance-totals-rows) 1)
        (setq dance-totals-rows (remove (assoc couple dance-totals-rows) dance-totals-rows)
              ...
              placed-couples (append placed-couples (list couple)))
  ...
  (defun remove-tied (tied-couples)
    (setq placed-couples (append placed-couples (list (couples-list tied-couples))))
  ...
  (defun filter-columns (filter-couples columns)
    ...
    (let ((edited (transpose-list columns)))
      (transpose-list (mapcar (lambda (x) (assoc x edited)) (couples-list filter-couples)))))
  ...
  (defun score-single-dance (dance)
    ...
    (print-standings placed-couples))
  ...
  (defun build-rule-11 (couples-list)
    (setq final-dance (apply 'append (mapcar 'symbol-value dance-symbols))
          ...
          starting-place (1+ (length placed-couples))
  ...
  (defun multi-dance (results-symbols)
    ...
    (loop with tied-couples = nil
          ...
          if (null tied-sum)
          return placed-couples
          if (= (length placed-couples) number-of-places)
          return placed-couples
          do (setq tied-couples (couples-with-sum final-couple-sums tied-sum))
          (build-rule-11 (make-couples-list-from-dotted-pair tied-couples))
          (score-multi-dance starting-place number-of-places (make-couples-list-from-dotted-pair tied-couples))
          (update-sums placed-couples final-couple-sums)
          (setq x (length placed-couples))))
