 ...
 #+BEGIN_SRC emacs-lisp
   (defun really-tied (tied-couples place)
     (loop for couple-1 in tied-couples
           ...
           do (remove-couples tied-couples)
           if (-all-p (lambda (x) (= (cdr x) (cdr couple-1))) tied-couples)
           do (loop with comparing-columns = (filter-columns tied-couples dance-totals-columns)
                    for place from place to 7
                    do (elt comparing-columns place))))
 #+END_SRC
