...
**** Set couple scores:
 #+BEGIN_SRC emacs-lisp :var cha-cha=cha-cha-results swing=swing-results rumba=rumba-results mambo=mambo-results
   (setq dances-couple-scores (append cha-cha rumba swing mambo)
         ...
         dance-totals-columns (create-dance-columns totals couples)
         dance-totals-rows (transpose totals-column))
 #+END_SRC
...
**** Procedural 
***** Step 1:
      Remove all couples in the order of lowest to highest until we have a tie
     #+BEGIN_SRC emacs-lisp
       (loop for (couple sum) in final-couple-sums
