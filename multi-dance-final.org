...
** Multi Dance:
*** Swap the car and cdr for convenience:
   #+BEGIN_SRC emacs-lisp
     (require 'cl)
...
*** Build the values of the scores at for each place
   #+BEGIN_SRC emacs-lisp
     (defun final-scores (couples-list)
...
#+END_SRC 
*** Sum the values for each couples
   #+BEGIN_SRC emacs-lisp :results value
     (defun final-sum-values (rows)
       (mapcar (lambda (couple) (list (car couple) (apply '+ (cdr couple)))) rows))
   #+END_SRC 
...
#+BEGIN_SRC emacs-lisp
  (final-sum-values final-couple-rows) 
#+END_SRC
...
**** Build Data:
 ...
 (defun calculate-totals (dance number-of-places)
    ...
    (defun create-dance-columns (totals-val couples-list)
      (append (list couples-list)(loop with sum-row = (make-list (length (first totals)) 0)
                                   for row in (butlast totals-val)
                                   do (setq sum-row (++ row sum-row))
                                   collect sum-row)))
...
#+BEGIN_SRC emacs-lisp :var cha-cha=cha-cha-results swing=swing-results rumba=rumba-results mambo=mambo-results
  (setq dances-couple-scores (append cha-cha rumba swing mambo)
        couple-scores (final-scores couples)
        final-couple-columns (transpose final-couple-rows)
