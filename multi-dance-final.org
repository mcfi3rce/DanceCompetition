...
#+name: place-value
   #+BEGIN_SRC emacs-lisp :results value
  (defun simple-majority (columns place)
      (loop with number-majorities = nil
            for row in (shorten columns place)
            for place-val from place to 7
            do (setq number-majorities (find-number-of-majorities row))
            when (= (length number-majorities) 1)
            do (remove-couple (caar number-majorities))
            (return (1+ place-val))))
#+END_SRC 
...
#+BEGIN_SRC emacs-lisp
  (simple-majority dance-totals-columns 1)
#+END_SRC
#+RESULTS:
: 3
...
#+BEGIN_SRC emacs-lisp 
  (defun complex-majority (columns place)  
    (loop with number-majorities = nil
          with tied-majorities = nil
          with couple-sums = nil
          with winning-couple = nil
          ;; this is to account for the removed rows
          for row in (shorten columns place)
          for place-val from place to 7
          do (setq number-majorities (find-number-of-majorities row))
          (case (length number-majorities)
            ...
            (t (setq tied-majorities (find-couples-with-largest-majority number-majorities))
               (case (length tied-majorities)
                 (1 (remove-couple (caar number-tied)))
               ;; tie-break 2 sums 
                 (t (setq couple-sums (compare-n-sums tied-majorities place-val)
                    ...
                    (case (length winning-couple)
                      (1 (remove-couples couple-sums)
                         (return (1+ place-val )))
                      (t (return couple-sums)))))))))
#+END_SRC
#+RESULTS:
: complex-majority

#+BEGIN_SRC emacs-lisp
  (complex-majority dance-totals-columns 3)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp 
  (simple-majority dance-totals-columns 5)
#+END_SRC
