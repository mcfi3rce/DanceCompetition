...
** DONE Initial values for the Dance:
   CLOSED: [2018-03-08 Thu 14:04]
 #+tblname: dances
...
** DONE Extract the place values for each of the dances:
   CLOSED: [2018-03-08 Thu 14:04]
 #+RESULTS: totals
...
** DONE Calculate the number of place values for each place:
   CLOSED: [2018-03-08 Thu 14:04]
 #+name: totals-row
...
** DONE Find-number-of-majorities Iterative Version:
   CLOSED: [2018-03-08 Thu 14:04]
#+BEGIN_SRC emacs-lisp
...
#+BEGIN_SRC emacs-lisp
  (defun really-tied (tied-couples place rows)
    (loop for couple-1 in tied-couples
          for couple-2 in (rest tied-couples)
          if (< (cdr couple-1)(cdr couple-2))
          return (remove-couples tied-couples (transpose rows))
          if (-all-p (lambda (x) (= x (cdr couple-1))) tied-couples)
          do (single-dance (filter-columns tied-couples dance-totals-columns))))
#+END_SRC
    ...
    (defun single-dance (dance-columns)
          (loop with dance = (cdr dance-columns)
                ...
                (case (length majority-couples)
                  (0)
                  (1 (setq dance (remove-couple (caar majority-couples) (transpose dance))))
                  (t (setq tied-majorities (find-couples-with-largest-majority majority-couples))
                     (case (length tied-majorities)
                       (1 (setq dance (remove-couple (caar tied-majorities) (transpose dance))))
...
#+BEGIN_SRC emacs-lisp :results silent 
  ...
  (defun remove-couples (couples rows)
   (mapcar (lambda (couple) (setq rows (remove-couple couple rows))) (mapcar 'car couples))rows)
...
**** couple 115 has a majority of 5
   #+BEGIN_SRC emacs-lisp 
     (loop for row in (cdr dance-totals-columns)
           collect (find-number-of-majorities row couples))
#+END_SRC 
...
*** Step 2: If there is only one couple then remove that couple
   #+BEGIN_SRC emacs-lisp 
     (setq dance-totals-rows (remove-couple 115 dance-totals-rows))
   #+END_SRC
...
#+BEGIN_SRC emacs-lisp 
  (loop for row in (cdr dance-totals-rows)
        collect (find-number-of-majorities row couples))
#+END_SRC 
...
#+BEGIN_SRC emacs-lisp
  (really-tied '((113 . 6)(114 . 7)) 3 dance-totals-rows)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp 
  (remove-couple 113 (transpose dance-totals-rows))
#+END_SRC
