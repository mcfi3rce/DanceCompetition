 ...
 #+BEGIN_SRC emacs-lisp :var dances=dances :results value
  ...
  (setq couples (mapcar 'first dances)
        ...
        dance-totals-columns (loop with sum-row = (make-list (length (first totals)) 0)
                                   ...
                                   collect sum-row)
        dance-totals-rows (transpose (append (list couples) dance-totals-columns))
        ; This is added because of difficult of dealing with the table
...
*** This will follow the way the final code will be evaluated:
    Step 1:
    Find the couple with the largest majority
   #+BEGIN_SRC emacs-lisp 
...
| (111 . 5) | (112 . 5) | (113 . 5) | (114 . 5) | (115 . 5) | (116 . 5) | (117 . 5) |
   ...
   Step 2:
   If there is only one couple then remove that couple
   #+BEGIN_SRC emacs-lisp 
     (remove-couple 115)
   #+END_SRC
...
|   5 |   5 |   5 |   5 |   5 |   5 |
   ...
   : 2
   Step 4: 
   Find the next couple with the largest majority
    #+BEGIN_SRC emacs-lisp 
      (loop for row in (cdr dance-totals-columns)
            collect (find-number-of-majorities row))
#+END_SRC 
:BUG:
We need to remove the couple number from the list of couples because it is
Throwing off our display
:END:

#+RESULTS:
| (113 . 3) | (114 . 3) |           |           |           |           |
| (112 . 3) | (113 . 3) | (114 . 4) | (1 . 3)   |           |           |
| (112 . 3) | (113 . 5) | (114 . 5) | (1 . 4)   | (116 . 3) |           |
| (112 . 5) | (113 . 5) | (114 . 5) | (1 . 5)   | (116 . 5) |           |
| (111 . 5) | (112 . 5) | (113 . 5) | (114 . 5) | (1 . 5)   | (116 . 5) |

   Step 5: Find next majority
  #+BEGIN_SRC emacs-lisp
    (largest-majority '((113 . 3)(114 . 3)))
#+END_SRC 
