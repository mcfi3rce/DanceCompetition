...
**** My attempt at recreating the rows from what we have
...
***** TODO Build the table for rule 11
...
#+BEGIN_SRC emacs-lisp 
  (defun make-final-dance-row (couples dances)
...
#+RESULTS:
: make-final-dance-row

#+BEGIN_SRC emacs-lisp
  (setq final-dance-rows (make-final-dance-row '(112 116) final-dance)
        final-dance-columns (transpose final-dance-rows)
        majority (1+ (/ (length (cdr (first final-dance-rows))) 2))
        number-of-places (1+ (length couples))
        starting-place 4
        totals (calculate-totals final-dance-rows number-of-places)
        final-totals-columns (create-dance-columns totals '(112 116))
        final-totals-rows (transpose final-totals-columns))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (loop for place from starting-place to number-of-places
        do (single-complex-majority (elt final-totals-columns place) place '(112 116)))
#+END_SRC
...
***** Remove the remaining dances
    ...
    #+RESULTS:
    | 115 | 113 | 114 | 116 | 112 | 117 | 111 | 111 |
***** Print the outcome
     #+BEGIN_SRC emacs-lisp
       (print-standings places)
#+END_SRC 
