...
*** DONE Check if all majority couples have been placed
    CLOSED: [2018-12-22 Sat 12:19]
 #+BEGIN_SRC elisp
...
**** Build Rule 10 Scores:
 #+BEGIN_SRC emacs-lisp
   ...
   (setq dance-symbols (list 'cha-cha 'rumba 'swing 'mambo))
#+END_SRC
...
: This creates the header to load in all the dances it isn't necessary but cool
#+BEGIN_SRC emacs-lisp :results raw
  (concatenate 'string "#+BEGIN_SRC elisp :var "
               (mapconcat (lambda (dance) (concatenate 'string dance "=" dance)) dances " "))
#+END_SRC

#+RESULTS:
: Try passing these values into the single-complex majority
#+BEGIN_SRC emacs-lisp
  (build-rule-11 couples)
...
#+RESULTS:
| 112 | 0 | 0 | 0 |  0 |  0 |  3 | 20 |
| 116 | 1 | 1 | 4 |  9 | 13 | 20 | 20 |
| 117 | 1 | 1 | 4 | 10 | 16 | 20 | 20 |
| 111 | 0 | 3 | 5 |  6 | 12 | 17 | 20 |
